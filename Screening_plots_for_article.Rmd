...
```{r}
read_tsv("Vdr 88-17 ANC300 till Claudia_bara_langa.txt") %>%
  ...
  rename(Sulphate=`SO4* mekv/l`) %>% 
  #clean_names() %>% 
  drop_na(Sulphate) %>% 
  pivot_longer(cols=c("Sulphate", "pH")) %>% 
  #drop_na(value) %>% 
...
plot_individual_trend(y = "Sulphate")
```
...
```{r}
screenmodel_out %>% filter(name=="Sulphate")%>% mutate(order=desc(as.numeric(str_sub(SWEREFID, start=1, end=7))))%>% plot_screeningtrends_reference(sorting = order, y_id = SWEREFID, wrappingvar = name)
```
...
```{r}
test_out%>%
  filter(SWEREFID=="7334144-846887", name=="Sulphate")%>%
plot_individual_trend(y = "Sulphate")
```
...
```{r}
screenmodel_out %>% filter(name=="Sulphate")%>% mutate(order=desc(as.numeric(str_sub(SWEREFID, start=1, end=7))))%>% plot_screeningtrends_relative(sorting = order, y_id = SWEREFID, wrappingvar = name)
```
